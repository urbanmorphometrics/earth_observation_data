...
def new_blocks(tessellation, edges, buildings, id_name="bID", unique_id="uID"):
    """Generate tessellation-based blocks
    """
    blocks_single = (
        gpd.overlay(
            tessellation,
    ...
    cells_copy = tessellation[[unique_id, "geometry"]].merge(
        centroids_tempID[[unique_id, id_name]], on=unique_id, how="left"
    )
    blocks = cells_copy.dissolve(by=id_name).explode().reset_index(drop=True)
